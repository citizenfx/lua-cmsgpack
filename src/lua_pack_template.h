...
*/
  ...
  #define _vectorpack_load32(cast, from, to) do {       \
        memcpy((cast*)(to), (from), sizeof(cast)); \
        *(to) = (cast)_vectorpack_be32(*(to));        \
    } while (0);

  #define _vectorpack_load64(cast, from, to) do {       \
        memcpy((cast*)(to), (from), sizeof(cast)); \
        *(to) = (cast)_vectorpack_be64(*(to));        \
    } while (0);

  #define _vectorpack_store32(to, num) \
      do { uint32_t val = _vectorpack_be32(num); memcpy(to, &val, 4); } while(0)

  #define _vectorpack_store64(to, num) \
      do { uint64_t val = _vectorpack_be64(num); memcpy(to, &val, 8); } while(0)
